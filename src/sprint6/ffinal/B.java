...
карта выстроена не оптимально.
В  случае отсутствия циклов проверяем на наличие белых вершин,  чтобы ещё раз  сделать обход.
...
-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
Граф представлен списками смежности, каждую вершину посещаем  1 или более раз
Несмотря на то, что мы проходим граф несколько раз в некоторых случаях,
с учетом алгоритма обхода данное количество не может быть очень большим,
скорее всего оно не превышает 2 прохода. Поэтому здесь не появляется O(V^2), сложность остается O(V).
