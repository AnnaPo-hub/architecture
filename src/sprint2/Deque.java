package sprint2;

import java.io.*;
 /*
    -- ПРИНЦИП РАБОТЫ --
 Используемая структура данных : массив фиксированной длины
 Массив используется в виде кольцевого буфера. С помощью указателей на элементы массива
 представляются ссылки на конец и начало дек. Чтобы добавить , мы сохраняем элемент в пустую ячейку
 куда указывает указатель начала дека, а указатель передвигаем вперед. При удалении элемента, сохраняем null в ячейку,
 которая указывет на конец дека, указатель передвигаем назад.
 Изменение значения указателей реализовано с учётом структуры кольцевого буфера.
 Выполняется автоматический переход от начала к концу и наоборот.

-- ДОКАЗАТЕЛЬСТВО КОРРЕКТНОСТИ --
push_front: Элемент будет добавлен в начало дэка, тк указатель показывает на пустой элемент в начале массива и
 передвигается вперед после добавлении элементов
pop_front: Элемент будет извлечен из начала  дэка, тк указатель будет передвинут назад  и затем
 элемент будет извлечен.
push_back: Элемент будет добавлен в конец дэка, тк указатель показывает на пустой элемент в  конце массива и
 передвигается  назад после добавлении элементов.
pop_back: Элемент будет извлечен из конца дэка, тк указатель  будет сначала передвинут , а затем
элемент будет извлечен.

-- ВРЕМЕННАЯ СЛОЖНОСТЬ --
Сложность O(n),  где  n- это количество команд.

-- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
O(n)  памяти, где n - это максимальное количество элементов в деке, передаваемое в качестве параметра.

--ID успешной посылки--
https://contest.yandex.ru/contest/22781/run-report/114200709/
     */

public class Deque {
    private Integer[] deque;
    private int head;
    private int tail;
    private int max_n;
    private int size;

    public Deque(int n) {
        deque = new Integer[n];
        head = 0;
        tail = n - 1;
        max_n = n;
        size = 0;
    }

    public boolean push_front(int x) {
        if (size != max_n) {
            deque[head] = x;
            head = (head + 1) % max_n;
            size++;
            return true;
        } else {
            return false;
        }
    }

    public String pop_front() {
        if (size == 0) {
            return "error";
        } else {
            head = (head - 1 + max_n) % max_n;
            Integer x = deque[head];
            deque[head] = null;
            size--;
            return x.toString();
        }
    }

    public boolean push_back(int x) {
        if (size != max_n) {
            deque[tail] = x;
            tail = (tail - 1 + max_n) % max_n;
            size++;
            return true;
        } else {
            return false;
        }
    }

    public String pop_back() {
        if (size == 0) {
            return "error";
        } else {
            tail = (tail + 1) % max_n;
            Integer x = deque[tail];
            deque[tail] = null;
            size--;
            return x.toString();
        }
    }

    public static void main(String[] args) throws IOException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
            final int n = Integer.parseInt(reader.readLine());
            final int max_n = Integer.parseInt(reader.readLine());

            Deque deque1 = new Deque(max_n);
            for (int i = 0; i < n; i++) {
                final String[] command = reader.readLine().trim().split(" ");

                switch (command[0]) {
                    case "push_front":
                        final boolean pushFront = deque1.push_front(Integer.parseInt(command[1]));
                        if (!pushFront) {
                            System.out.println("error");
                        }
                        break;
                    case "push_back":
                        final boolean pushBack = deque1.push_back(Integer.parseInt(command[1]));
                        if (!pushBack) {
                            System.out.println("error");
                        }
                        break;
                    case "pop_front":
                        System.out.println(deque1.pop_front());
                        break;
                    case "pop_back":
                        System.out.println(deque1.pop_back());
                        break;
                }
            }
        }
    }
}